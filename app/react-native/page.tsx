...
export default async function Page() {
  let moduleData: any;
  try {
    moduleData = await fetchModuleContent('react-native');
  } catch (err) {
    const message = err instanceof Error ? err.message : 'Unknown error';
    return (
      <ModuleLayout
        title="React Native"
        description="Cross-platform mobile development and native integrations"
        icon="📱"
        sections={[{ id: 'overview', title: 'Overview' }]}
      >
        <section id="overview" className="mb-12">
          <ApiErrorState title="Backend API required" message={`Failed to load module 'react-native' from API: ${message}`} />
        </section>
      </ModuleLayout>
    );
  }
    ...
    >
      <section id="overview" className="mb-12">
        <div className="bg-gradient-to-br from-emerald-500/10 to-teal-500/10 border border-emerald-500/20 rounded-2xl p-8">
          <h2 className="text-2xl font-bold text-white mb-4">Why React Native Matters</h2>
          <p className="text-slate-300 mb-4">
            This module focuses on the fundamentals and interview-critical concepts for React Native. Use it to build
            strong mental models, sharpen terminology, and practice common interview questions.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
            <div className="bg-slate-900/50 rounded-lg p-4">
              <h3 className="font-semibold text-emerald-400 mb-2">Concepts</h3>
              <p className="text-sm text-slate-400">Core ideas and patterns you will use daily</p>
            </div>
            <div className="bg-slate-900/50 rounded-lg p-4">
              <h3 className="font-semibold text-emerald-400 mb-2">Practical Skills</h3>
              <p className="text-sm text-slate-400">Hands-on knowledge for real projects</p>
            </div>
            <div className="bg-slate-900/50 rounded-lg p-4">
              <h3 className="font-semibold text-emerald-400 mb-2">Interview Focus</h3>
