...
import { nodejsData } from './data';
import { sortByDifficultyHardFirst, sortByImportanceHardFirst, sortTopicsHardFirst } from '../lib/rank';

export default function Page() {
  const technicalTerms = [...nodejsData.technicalTerms].sort(sortByImportanceHardFirst);
  const topics = [...nodejsData.topics].sort(sortTopicsHardFirst);
  const questions = [...nodejsData.questions].sort(sortByDifficultyHardFirst);

  return (
    <ModuleLayout
      title="Node.js"
      description="Event loop, streams, and building scalable server applications"
      icon="🟢"
      sections={[
    ...
    >
      <section id="overview" className="mb-12">
        <div className="bg-gradient-to-br from-green-500/10 to-emerald-500/10 border border-green-500/20 rounded-2xl p-8">
          <h2 className="text-2xl font-bold text-white mb-4">Why Node.js Matters</h2>
          <p className="text-slate-300 mb-4">
            This module focuses on the fundamentals and interview-critical concepts for Node.js. Use it to build
            strong mental models, sharpen terminology, and practice common interview questions.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
            <div className="bg-slate-900/50 rounded-lg p-4">
              <h3 className="font-semibold text-green-400 mb-2">Concepts</h3>
              <p className="text-sm text-slate-400">Core ideas and patterns you will use daily</p>
            </div>
            <div className="bg-slate-900/50 rounded-lg p-4">
              <h3 className="font-semibold text-green-400 mb-2">Practical Skills</h3>
              <p className="text-sm text-slate-400">Hands-on knowledge for real projects</p>
            </div>
            <div className="bg-slate-900/50 rounded-lg p-4">
              <h3 className="font-semibold text-green-400 mb-2">Interview Focus</h3>
      ...
      <section id="technical-terms" className="mb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {technicalTerms.map((term) => (
            <div
              key={term.term}
              className="bg-slate-900 border border-slate-800 rounded-xl p-5 hover:border-green-500/30 transition-colors"
            >
              <h3 className="text-lg font-semibold text-green-400 mb-2">{term.term}</h3>
